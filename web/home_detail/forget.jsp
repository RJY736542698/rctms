    ...
    <script type="text/javascript">
    	$(function(){
    		//页面初始化，查询当前用户信息
  			$.ajax({
				...
				success:function(result){
					...
					}else if(result.message=="error"){  //没登陆
    				...
    				$.ajax({
    					...
    					success:function(result){
    						if(result.message=="success"){ //通过
    							...
    							$(".msg").eq(0).text("验证码已过期，请重新发送")
    							$("#code").html("");
    						}else if(result.message=="error"){ //验证未通过
    							$(".msg").eq(0).text("验证码不正确")
    							$("#code").html("");
    						}
    					}
    				});
    			}else{
    				$(".msg").eq(0).text("验证码不许为空")
    			}
    		});
    		...
    		$("#but2").click(function(){
    			...
    			var reg=/^(.){6,20}$/;
    			if(password==""){
    				$(".msg").eq(1).text("密码不允许为空")
    			}
    			else if(reg.test(password)==false){
    				$(".msg").eq(1).text("不符合规则，满足6-20位")
    			}
    			else if(password!=password1){
    				$(".msg").eq(1).text("两次密码输入不一致")
    			}
    			else{
    					//修改密码
    					$.ajax({
...
<body>
    ...
    <div class="modalfom setpass container">
        ...
        <form  class="row">
            ...
            <div class="row">
                <div class="col-sm-11">
                    <button type="button" id="but2" class="btn btn-primary btn-lg btn-block">确 认</button>                    
                </div>
            </div>
        </form>
    </div>  
    ...
    <div class="modalfom success container">
        ...
        <div class="row">
            ...
            <div class="col-sm-12">
                <button type="button" class="btn btn-primary btn-lg btn-block" id="but3">确 定</button>                    
            </div>
        </div>
    </div>  
    <!--底部-->
